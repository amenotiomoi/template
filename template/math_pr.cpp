...
namespace PR {
   ...
   void Fac_dfs(ll x,ll y,vector<ll>& ans,vector<pair<ll,ll>> FacList){if(x==(ll)(FacList.size())){ans.push_back(y);return;}ll k=1;for(ll i=0;i<=FacList[x].second;++i){Fac_dfs(x+1,y*k,ans,FacList);k*=FacList[x].first;}}
	vector<pair<ll,ll>> f(vector<ll> Q) {vector<pair<ll,ll>> P;sort(Q.begin(),Q.end());for(ll i:Q)if(P.empty()||P.end()[-1].first!=i) P.push_back({i,1});else P.end()[-1].second++;return P;}
   ...
   vector<ll>GetFacList(ll x){vector<ll> ans;Fac_dfs(0,1,ans,f(PrimeFacList(x)));return ans;}
};
