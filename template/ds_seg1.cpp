template <class node> class segment1 {
   private:
      int len,siz;node* tree;
   public:
      segment1(int n,node y){len=n;siz=(1<<(__lg(n)+1));tree=new node[siz+n+2];for(int i=(len+siz)/2+1;i<=siz+n+1;++i)tree[i]=y;for(int i=(len+siz)/2;i>=1;--i)tree[i]=node::merge(tree[i*2],tree[i*2+1]);}
